...
Before starting, resolve the source of information in this order:

1. **Jira MCP available** and argument matches Jira ID format (e.g. `SCRUM-42`): use the MCP to get full ticket details.
2. **MCP not available or not responding**: look for local file `ai-specs/changes/[ID]_ticket.md` or `ai-specs/changes/[ID]_input.md`. If found, read and use it as ticket description.
3. **No source available**: tell the user what information is needed and ask them to paste the ticket description directly in the chat. Continue when received.

> ⚠️ Do not proceed without resolving the ticket context.
...
### 2. **Implementation Status** *(updated when implemented)*
```
